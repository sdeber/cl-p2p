...
(defun initialize (ip id &optional (port 7000))
  (let* ((node (make-instance 'chord-node)))
    ...
    (sb-bsd-sockets:socket-listen (get-socket node) 20)
    node))
...
(defmethod send-message ((sender node) (destination node) content type &optional (sock nil))
  (let ((message (make-instance 'message
	...
	(with-socket (sock :stream :tcp)
	  (sb-bsd-sockets:socket-connect sock (get-ip destination)
...
(defmethod send-and-get-reply ((peer chord-node) content type known-ip &optional (port 7000))
  (let ((message (create-message (get-id peer)
	...
	(reply nil))
    (handler-case  (progn (with-socket (sock :stream :tcp)
				...
				(loop 
				   ...
				   do (progn (sb-thread:condition-wait
					     ...
					     (let ((data (gethash content (get-pool pool))))
					       (if (not (eql data t))
						   (setf reply data))))))
			      (remove-data pool content))))
      (timeout () (remove-data pool content)))
...
(defun process-message (peer sock)
  (handler-case 
      (let ((myself (get-local peer))
	    (my-id (get-id peer))
	...
	(labels ((process-search (message)
		   (let* ((sender (get-sender message))
			 ...
			 (send-message myself sender key :answer sock)
		 ...
		 (search-successor (message)
		   (if (or (in-interval (get-id (get-predecessor peer))
		       ...
		       (send-message myself (get-sender message) (get-content message) :answer sock)
		       (forward-message message peer)))
