...
(defun initialize (ip id &optional (port 7000))
  "Create a chord node and initialize it"
...
(defclass peer ()
  ((ipAddr :accessor get-ip)
   (port :accessor get-port)))

(defclass chord-peer (peer)
  ((id :accessor get-id)
   (fingertable :accessor get-fingertable)
   (peer-cache :accessor get-cached-peers)
   (sock :accessor get-socket)))


(defun join-network (host port)
  (let ((sock (usocket:socket-connect host port :protocol :datagram :timeout 10)))
    ))
(defmethod get-successor ((peer chord-peer))
  )
(defgeneric send-message (message dist)
  (:documentation "Send a message to a target"))

(defclass chord-node (peer)
  ((fingertable :accessor get-fingertable
...
(defmethod send-and-get-reply ((peer chord-node) content type known-ip &optional (port 7000))
  "send a message and wait for the reply, it will timeout after 5 seconds"
  (let* ((message (create-message (get-id peer)
	 ...
	 (waiting-queue (get-waiting-queue pool))
	 (lock (get-lock pool))
	 (result nil))
    (with-socket (sock :datagram :udp)
...
(defmethod lookup ((peer chord-node) key)
  "search for a key, it returns the peer who is responsible for that key"
...
(defmethod server ((peer chord-node))
  (let ((requestpool (get-requestpool peer))
    ...
    (labels ((process-search (message)
		 ...
		 (when (not (null (gethash (get-content message)
					   (get-pool requestpool))))
	     ...
	     (process-notify (message)
	       (if (or (in-interval (get-id (get-predecessor peer))
				    my-id
				    (get-id (get-sender message)))
		       (equalp (get-id (get-predecessor peer)) my-id))
		   (setf (get-predecessor peer) (get-sender message))))
	     (process-quit (message)
	       (if (equalp (get-id (get-successor peer))
			   (get-content message))
		   (setf (aref (get-fingertable peer) 1) (get-sender message))
		   (if (equalp (get-id (get-predecessor peer))
			       (get-content message))
		       (setf (get-predecessor peer) (get-sender message))))))
	    ...
	    (sb-thread:make-thread
	     (lambda ()
		  ...
		  do (let ((m (vector->message message)))
			 ...
			 (:quit (process-quit m))
			 (:notify (process-notify m)))))))))))
		    
(defmethod join ((node chord-node) known-ip &optional (port 7000))
  "join a chord system by connecting to a known peer"
...
(defmethod leave ((node chord-node))
  (send-message (get-predecessor node) (get-successor node) (get-id node) :quit)
  (send-message (get-successor node) (get-predecessor node) (get-id node) :quit))


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;The following functions are used for debugging only. 
